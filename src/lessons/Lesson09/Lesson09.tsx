...
function Lesson09() {
  ...
  return (
    <div className='lesson-container'>
      ...
      <ol>
        <li>cтрелочную функцию с действиями, которые вы хотите запускать 
            при определенных условиях </li>
        <li>массив зависимостей. В нем через запятую можно указать 
            от изменения каких переменных будет зависеть запуск действий. 
            Если массив пустой - код сработает только при монтировании (mounting)
             компонента</li>
      </ol>
      <p>Мы будем использовать хук useEffect() для получения данных из 
        асинхронных запросов с пустым массивом зависимостей</p>
      <p>Любой изменение стейта вызывает ререндр компонента. Это значит, что все 
        операции в теле компонента будут повторяться раз за разом. В том числе и те,
         которые мы бы не хотели повторять. </p>
      <p>
        Нужен способ чтобы мы могли изолировать операции от повтора там, 
        где нам это не нужно.
      </p>
